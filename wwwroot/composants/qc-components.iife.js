(function(){"use strict";class h extends HTMLElement{static get observedAttributes(){return["label","type","href","class","display","size","icon","icon-position"]}constructor(){super(),this.render()}attributeChangedCallback(e,t,s){t!==s&&this.render()}get template(){const e=this.className?this.className+" "+this.currenttype:this.currenttype+" "+this.currentDisplay;return`
    ...
    `}}customElements.get("qc-footer")||customElements.define("qc-footer",y);class A extends HTMLElement{static get observedAttributes(){return["titre"]}attributeChangedCallback(e,t,s){t!==s&&this.render()}constructor(){super(),this.render()}connectedCallback(){this.render(),this.setupAnchors()}get template(){return`
        ...
        `}render(){this.innerHTML=this.template}get titre(){return this.getAttribute("titre")||"Table des matières"}generateAnchors(){const e=[];return document.querySelectorAll(".tdm").forEach((s,i)=>{const c=s.getAttribute("tdm-titre")||s.textContent.trim(),n=`tdm-anchor-${i}`;s.id=n,e.push(`<li><a href="#${n}" class="tdm-link">${c}</a></li>`)}),e.join("")}setupAnchors(){this.querySelectorAll(".tdm-link").forEach(t=>{t.addEventListener("click",s=>{s.preventDefault();const i=t.getAttribute("href").substring(1),c=document.getElementById(i);c&&c.scrollIntoView({behavior:"smooth"})})})}}customElements.get("qc-tdm")||customElements.define("qc-tdm",A);class k extends HTMLElement{constructor(){super(),this.render()}connectedCallback(){window.addEventListener("scroll",this.toggleVisibility.bind(this)),this.querySelector("qc-bouton").addEventListener("click",this.scrollToTop)}disconnectedCallback(){window.removeEventListener("scroll",this.toggleVisibility.bind(this)),this.querySelector("qc-bouton").removeEventListener("click",this.scrollToTop)}render(){this.innerHTML=`
      <qc-bouton class="qc-up-btn" label="Défiler vers le haut" icon="lnr-arrow-up" icon-position="left"></qc-bouton>
    `}toggleVisibility(){const e=this.querySelector("qc-bouton");window.scrollY>300?e.style.display="block":e.style.display="none"}scrollToTop(){window.scrollTo({top:0,behavior:"smooth"})}}customElements.get("qc-up")||customElements.define("qc-up",k);class C extends HTMLElement{static get observedAttributes(){return["titre","medias","theme"]}constructor(){super(),this.render()}get template(){const e=this.theme==="light"?"theme-light":"theme-dark",s=JSON.parse(this.medias||"[]").map(i=>`
            <li>
        ...
        `}}customElements.get("qc-ariane")||customElements.define("qc-ariane",E);class M extends HTMLElement{static get observedAttributes(){return["label"]}constructor(){super(),this.render()}attributeChangedCallback(e,t,s){t!==s&&this.render()}get template(){return`
        <div class="qc-tag">
            ${this.tag}
        </div>
    `}get label(){return this.getAttribute("label")||"Étiquette"}render(){this.innerHTML=this.template}}customElements.get("qc-etiquette")||customElements.define("qc-etiquette",M);class $ extends HTMLElement{static get observedAttributes(){return["tag"]}attributeChangedCallback(e,t,s){t!==s&&this.render()}constructor(){super(),this.render()}get template(){return`
            <div class="qc-tag">
                ${this.tag}
            </div>
        `}get tag(){return this.getAttribute("tag")}render(){this.innerHTML=this.template}}customElements.get("qc-tag")||customElements.define("qc-tag",$)})();
