(function(){"use strict";class h extends HTMLElement{static get observedAttributes(){return["label","type","href","class","display","size","icon","icon-position"]}constructor(){super(),this.render()}attributeChangedCallback(e,t,s){t!==s&&this.render()}get template(){const e=this.className?this.className+" "+this.currenttype:this.currenttype+" "+this.currentDisplay;return`
    ...
    `}}customElements.get("qc-footer")||customElements.define("qc-footer",y);class A extends HTMLElement{static get observedAttributes(){return["titre"]}attributeChangedCallback(e,t,s){t!==s&&this.render()}constructor(){super(),this.render()}connectedCallback(){this.render(),this.setupAnchors()}get template(){return`
        ...
        `}render(){this.innerHTML=this.template}get titre(){return this.getAttribute("titre")||"Table des matières"}generateAnchors(){const e=[];return document.querySelectorAll(".tdm").forEach((s,i)=>{const n=s.getAttribute("tdm-titre")||s.textContent.trim(),r=`tdm-anchor-${i}`;s.id=r,e.push(`<li><a href="#${r}" class="tdm-link">${n}</a></li>`)}),e.join("")}setupAnchors(){this.querySelectorAll(".tdm-link").forEach(t=>{t.addEventListener("click",s=>{s.preventDefault();const i=t.getAttribute("href").substring(1),n=document.getElementById(i);n&&n.scrollIntoView({behavior:"smooth"})})})}}customElements.get("qc-tdm")||customElements.define("qc-tdm",A);class k extends HTMLElement{constructor(){super(),this.render()}connectedCallback(){window.addEventListener("scroll",this.toggleVisibility.bind(this)),this.querySelector("qc-bouton").addEventListener("click",this.scrollToTop)}disconnectedCallback(){window.removeEventListener("scroll",this.toggleVisibility.bind(this)),this.querySelector("qc-bouton").removeEventListener("click",this.scrollToTop)}render(){this.innerHTML=`
    ...
    `}toggleVisibility(){const e=this.querySelector("qc-bouton");window.scrollY>300?e.style.display="block":e.style.display="none"}scrollToTop(){window.scrollTo({top:0,behavior:"smooth"})}}customElements.get("qc-up")||customElements.define("qc-up",k);class C extends HTMLElement{static get observedAttributes(){return["titre","medias","theme"]}constructor(){super(),this.render()}get template(){const e=this.theme==="light"?"theme-light":"theme-dark",s=JSON.parse(this.medias||"[]").map(i=>`
        ...
        `}}customElements.get("qc-navigation")||customElements.define("qc-navigation",L);class M extends HTMLElement{static get observedAttributes(){return["arialabel"]}constructor(){super(),this.attachShadow({mode:"open"}),this.render()}attributeChangedCallback(e,t,s){t!==s&&this.render()}render(){const e=`
            :host {
                --margin-top: 1rem;
                display: block;
            }
